...
from utils import get_grads, get_activation_gradients
from data.dataloader import load_train_data, decode, load_test_data


class InfluenceCalc:
    def __init__(self, model, tokenizer, mlp_blocks, dataset, layer):
        self.model = model
        self.layer_index = layer
        self.tokenizer = tokenizer
        self.mlp_blocks = mlp_blocks[:layer]
...
def main():
    ...
    model.eval()
    inf = InfluenceCalc(model, tokenizer, mlp_blocks, dataset, layer=2)
